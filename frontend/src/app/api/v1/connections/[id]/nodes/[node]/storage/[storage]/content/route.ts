...
export async function GET(
  ...
  try {
    ...
    // Browsing storage content is `storage.content`, not `vm.view` (issue
    // #920): the catalogue advertises the right, so clearing it from a role
    // has to take the content listing away. Roles that drive the guest
    // wizards need it too — their disk and ISO pickers read this route.
    ...
    const denied = await checkPermission(PERMISSIONS.STORAGE_CONTENT, "connection", id)

    if (denied && !(await guestPerimeterAllows(id, PERMISSIONS.STORAGE_CONTENT))) return denied
