...
/**
 * GET /api/v1/nutanix/[id]/vms
 * List VMs from Nutanix Prism Central
 */
export async function GET(
  ...
  try {
    ...
    const nutanixVms = await client.listVMs()

    // Map to the standard format expected by the frontend
    const vms = nutanixVms.map(vm => ({
      vmid: vm.uuid,
      name: vm.name,
      status: vm.powerState === 'ON' ? 'running' : 'stopped',
      cpu: vm.numCpus || undefined,
      memory_size_MiB: vm.memoryMB || undefined,
      power_state: vm.powerState,
      guest_OS: vm.osType || undefined,
      committed: vm.diskSizeBytes || undefined,
      clusterName: vm.clusterName || undefined,
    }))
